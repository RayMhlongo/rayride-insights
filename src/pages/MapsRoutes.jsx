...
import { Bus, Car, LocateFixed, MapPinned, Route as RouteIcon, Users } from 'lucide-react';
...
export default function MapsRoutes({ app }) {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Maps & Routes" description="OpenStreetMap view of routes, pickups, and assigned vehicles." />

      <div className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
        <aside className="panel order-2 p-4 xl:order-1">
          <h3 className="font-bold text-navy dark:text-slate-50">Map Controls</h3>
        ...
        <section className="panel order-1 overflow-hidden xl:order-2">
          <div className="h-[68vh] min-h-[520px] w-full overflow-hidden rounded-lg md:h-[calc(100vh-170px)]">
...
function buildMapData(data) {
  const routes = data.routes.map((route, index) => {
    ...
    const points = stopPoints.length >= 2 ? stopPoints : studentPoints.length >= 2 ? studentPoints : fallback;
    return { ...route, points, color: routeColors[index % routeColors.length] };
  });

  const students = data.students.map((student, index) => ({
    ...student,
    position: getRecordPosition(student, index, routeIndex(data.routes, student.routeId)),
  }));
  ...
  const vehicles = data.vehicles.map((vehicle, index) => {
    ...
    return {
      ...
      position: route?.points?.at(-1) || seedPosition(vehicle.id || vehicle.registration, index, 18),
    };
  });
